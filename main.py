...
def main():
    print('Welcome to Tic Tac Toe by Deeanshu!')
    printBoard(board)
    global m

    while not (isBoardFull(board)):
        if not (isWinner(board, cp)):
            if cp == 'o':
                if m == 1:
                    m -= 1
                    pass
            ...
            print('Computer won!! muhaahahaha!!?')
            break

        if not (isWinner(board, pp)):
            move = compMove()
            if move == 0:
                print('Tie Game!')
            else:
                insertLetter(cp, move)
                print('Computer placed  in position', move, ':')
                printBoard(board)
        else:
            print('He won papi :( ; See you next time. Good job btw')
            break
...
while True:
    m=1
    answer = input('Do you want to continue the game? (Y/N): ')
    if answer.lower() == 'y' or answer.lower == 'yes':
        op = input('Choose your Stuff(o/x): ')
        if op.lower() == 'o':
            pp = 'o'
            cp = 'x'
            board = [' ' for x in range(10)]
